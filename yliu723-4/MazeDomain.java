 ...
 *
 */
public class MazeDomain implements DomainGenerator {
	 ...
	 */
	public MazeDomain(int width, int height){
		this.width = width;
		this.height = height;
	...
	public class MovementAction extends Action{
		...
		protected State performActionHelper(State st, String[] params) {
			...
			int [] dcomps = MazeDomain.this.movementDirectionFromIndex(dir);
			MazeDomain.this.move(st, dcomps[0], dcomps[1]);
		...
		public List<TransitionProbability> getTransitions(State st, String [] params){
			...
			for(int i = 0; i < directionProbs.length; i++){
				...
				State ns = st.copy();
				int [] dcomps = MazeDomain.this.movementDirectionFromIndex(i);
				MazeDomain.this.move(ns, dcomps[0], dcomps[1]);
	...
	public class WallToPF extends PropositionalFunction{
		...
		public WallToPF(String name, Domain domain, String[] parameterClasses, int direction) {
			super(name, domain, parameterClasses);
			int [] dcomps = MazeDomain.this.movementDirectionFromIndex(direction);
			xdelta = dcomps[0];
			ydelta = dcomps[1];
		}
		...
		public boolean isTrue(State st, String[] params) {
			...
			if(cx < 0 || cx >= MazeDomain.this.width || cy < 0 || cy >= MazeDomain.this.height || MazeDomain.this.map[cx][cy] == 1 || 
					(xdelta > 0 && (MazeDomain.this.map[ax][ay] == 3 || MazeDomain.this.map[ax][ay] == 4)) || (xdelta < 0 && (MazeDomain.this.map[cx][cy] == 3 || MazeDomain.this.map[cx][cy] == 4)) ||
					(ydelta > 0 && (MazeDomain.this.map[ax][ay] == 2 || MazeDomain.this.map[ax][ay] == 4)) || (ydelta < 0 && (MazeDomain.this.map[cx][cy] == 2 || MazeDomain.this.map[cx][cy] == 4)) ){
				return true;
			}
	...
	public static void main(String[] args) {
		...
		MazeDomain gwdg = new MazeDomain(50, 50);
		gwdg.setMapToMaze();
